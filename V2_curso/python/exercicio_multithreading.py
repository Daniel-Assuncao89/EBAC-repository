...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        movie_data = movie_soup.find('div', attrs={'class': 'sc-69e49b85-0 jqlHBQ'})
        ...
        rating = movie_soup.find('div', attrs={'class': 'sc-bde20123-2 cdQqzc'}).get_text() if movie_soup.find(
            'span', attrs={'class': 'sc-bde20123-1 cMEQkK'}) else None

        # plot é o texto de sinopse do filme
        plot_text = movie_soup.find('p', attrs={'class': 'sc-466bb6c-3 fOUpWp'}).get_text().strip() if movie_soup.find(
